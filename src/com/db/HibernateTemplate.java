...
public class HibernateTemplate {
	...
	public static int updateQuizMarks(String studentId, String YBSId, String examType, int marks) {
		Session session=sessionFactory.openSession();
		Transaction tx = session.beginTransaction();
		Query query = null;
		if(examType.equals("mid1")) query = session.createQuery("update ScriptsAndMarks set quiz1Marks = :marks where studentId = :studentId and YBSId = :YBSId");
		else if(examType.equals("mid2")) query = session.createQuery("update ScriptsAndMarks set quiz2Marks = :marks where studentId = :studentId and YBSId = :YBSId");
	...
	public static List<Quiz> getQuiz(String YBSId, String examType){
		String queryString="from Quiz where YBSId = :YBSId and examType = :examType";
		Query query = sessionFactory.openSession().createQuery(queryString);
		query.setString("YBSId", YBSId);
	...
	public static int deleteStudents(int year){
		Session session = sessionFactory.openSession();
		Transaction tx =  session.beginTransaction();
		String queryString="delete from Student where currentYear = :year";
	...
	public static int updateAPaperPath(String YBSId, String examType, String studentId, String paperPath) {
		Session session=sessionFactory.openSession();
		Transaction tx = session.beginTransaction();  
		Query query = null;
		if(examType.equals("mid1")) query=session.createQuery("update ScriptsAndMarks set desc1Script = :paperPath where YBSId = :YBSId and studentId = :studentId");
		else if(examType.equals("mid2")) query=session.createQuery("update ScriptsAndMarks set desc2Script = :paperPath where YBSId = :YBSId and studentId = :studentId");
		else if(examType.equals("asgn1")) query=session.createQuery("update ScriptsAndMarks set asgn1Script = :paperPath where YBSId = :YBSId and studentId = :studentId");
		else if(examType.equals("asgn2")) query=session.createQuery("update ScriptsAndMarks set asgn2Script = :paperPath where YBSId = :YBSId and studentId = :studentId");
		else if(examType.equals("sem")) query=session.createQuery("update ScriptsAndMarks set semScript = :paperPath where YBSId = :YBSId and studentId = :studentId");
		query.setString("YBSId", YBSId);  
		query.setString("studentId", studentId);
		query.setString("paperPath", paperPath);
		int status = query.executeUpdate();   
		tx.commit();
		System.out.println(status); 
		return status;
	...
	public static Descriptive getExamTime(String YBSId, String examType) {
		String queryString="from Descriptive where YBSId = :YBSId and examType = :examType";
		Query query = sessionFactory.openSession().createQuery(queryString);
		query.setString("YBSId", YBSId);
		query.setString("examType", examType);
