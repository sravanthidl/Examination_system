...
public class SAMDao {
	...
	public int add(ScriptsAndMarks sam) {
		return HibernateTemplate.addObject(sam);
	}
	public int updateAPaperPath(String YBSId, String examType, String studentId, String paperPath) {
		return HibernateTemplate.updateAPaperPath(YBSId, examType, studentId, paperPath);
	}
	public int updateQuizMarks(String studentId, String YBSId, String examType, int marks) {
		ScriptsAndMarks sam = HibernateTemplate.getSam(studentId, YBSId);
		int x=0;
		if(examType.equals("mid1")||examType.equals("quiz2")) {
			x=HibernateTemplate.updateQuizMarks(studentId, YBSId, examType, marks);
		}else if(examType.equals("mid2")||examType.equals("quiz2")) {
			x=HibernateTemplate.updateQuizMarks(studentId, YBSId, examType, marks);
		}
		return x;
	}
